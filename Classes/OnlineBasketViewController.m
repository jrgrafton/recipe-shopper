...
@interface OnlineBasketViewController()
...
@implementation OnlineBasketViewController
...
- (void)viewDidLoad {
	...
	[onlineBasketView setAllowsSelection:NO];
}
...
- (void)viewWillAppear:(BOOL)animated {
	...
	[NSThread detachNewThreadSelector:@selector(updateOnlineBasketDetails) toTarget:self withObject:nil];
}
...
- (IBAction)transitionToDeliverySlotView:(id)sender {
	...
	[[appDelegate onlineBasketViewController] pushViewController:self.deliverySlotsViewController animated:YES];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == 0) {
		...
		NSArray *buttons = [UITableViewCellFactory createProductTableCell:&cell withIdentifier:ProductBasketCellIdentifier withProduct:product andQuantity:quantity];
...
- (BOOL)tabBarController:(UITabBarController *)tabBarController shouldSelectViewController:(UIViewController *)viewController {
	if (![DataManager loggedIn] && (viewController == [tabBarController.viewControllers objectAtIndex:3])) {
...
- (void)addProductButtonClicked:(id)sender {
	...
	while ((product = [productsEnumerator nextObject])) {
		if ([[product productBaseID] intValue] == [productBaseID intValue]) {
			...
			/* increase the item count and add the cost of one of these items to the basket price */
			...
			/* reload the product basket - the price and number of items count etc. will be updated once the 
...
- (void)removeProductButtonClicked:(id)sender {
	...
	while ((product = [productsEnumerator nextObject])) {
		if ([[product productBaseID] intValue] == [productBaseID intValue]) {
			...
			/* decrease the item count and deduct the cost of one of these items from the basket price */
			CGFloat productPrice = [[product productPrice] floatValue];
			CGFloat currentBasketPrice  = [[self basketPrice] floatValue];
			[self setBasketPrice:[NSString stringWithFormat:@"%.2f", currentBasketPrice - productPrice]];
...
- (void)updateOnlineBasketDetails {
	...
	NSDictionary *onlineBasketDetails = [DataManager getOnlineBasketDetails];
	[self setBasketPrice:[onlineBasketDetails objectForKey:@"BasketPrice"]];
	[self setBasketSavings:[onlineBasketDetails objectForKey:@"BasketSavings"]];
