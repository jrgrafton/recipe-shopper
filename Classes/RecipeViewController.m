...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
	...
	if (locationOfLink != NSNotFound) {
		...
		/* pop the recipe basket view back to the recipe basket (if its on the shopping list view etc.) */
		...
		if ([[appDelegate tabBarController] selectedIndex] != 1) {
			[[appDelegate recipeBasketViewController] popToRootViewControllerAnimated:NO];
		}
	}
